...
impl Order {
    pub fn new( product_name: String, quantity: u16, unit_price: i16) -> Self {
    ...
    pub fn total(&self)-> i32 {
        (self.quantity as i32) * (self.unit_price as i32)
    }
    ...
    fn validate_product_name(product_name: &String){
        ...
        if product_name.len() > 300 {
            panic!("Product name be longer than 50 bytes");
        }
    }

    fn validate_quantity(quantity: u16){
        if quantity == 0 {
            panic!("Quantity must be more than 0");
        }
    }

    fn validate_unit_price(unit_price: i16){
        if unit_price <= 0 {
            panic!("Unit price must be more than 0");
        }
    }
}